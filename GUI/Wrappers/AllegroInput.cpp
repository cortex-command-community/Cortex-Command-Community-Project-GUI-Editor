...
*/
    ...
    if (m_KeyJoyMouseCursor)
    {
        ...
        float acceleration = 0.25 + std::min(m_pCursorAccelTimer->GetElapsedRealTimeS(), 0.5) * 20;
        ...
        mouse_x = std::min((g_FrameMan.GetResX() * mouseDenominator) - 3, mouse_x);
        mouse_y = std::min((g_FrameMan.GetResY() * mouseDenominator) - 3, mouse_y);
...
void AllegroInput::ConvertKeyEvent(int allegroKey, int guilibKey, float elapsedS)
{
    if (key[allegroKey])
    {
        ...
        else
        {
            ...
            m_KeyboardTimes[guilibKey] = 0;//std::max(m_KeyboardTimes[guilibKey] - m_KeyDelay, 0);
        }
